...
class WidgetRecommendation extends WP_Widget
{
    ...
    public function widget($args, $instance)
    {
        ...
        $cache_field = MlRecommendationBlock::$cache_prefix . $this->cache_key_helper();

        // Кеширование
        $f_posts = get_transient($cache_field);

        if (false == $f_posts) {
            // Получаем все  посты кроме тех которые исключены по произвольному метаполю
            $s_posts = $this->get_posts($post_types, $instance['posts_per_page'], $instance['last_days'], $instance['exclude_posts_in_taxonomy']);
            // Получаем посты котрые включены метаполем
            $p_include = $this->get_posts_include($post_types, $instance['posts_per_page'], $instance['last_days']);

            if (!empty($p_include)) {
                // если есть включеные метаполя то сначалала сливаемиз со всеми постами
                // а потом обрезаем массив постов до заданной в параметрах виджета размеров
            ...
            if (!empty($f_posts)) {
                ...
                if (MlRecommendationBlock::$cache) {
                    // Кеширование на час
                    set_transient($cache_field, $f_posts, HOUR_IN_SECONDS);
                }
            }
        }
        ...
        if (0 < count($f_posts)):
            ...
            <div class="ml-recommendation__wrapper">
                <div class="ml-recommendation__wrap">
                    <div class="ml-recommendation__co-content">
                        <?php _e('No recommendation posts', 'MlRecommendationBlock'); ?>
                    </div>
                </div>
            </div>
        <?php endif;
    ...
    private function cache_key_helper()
    {
    ...
    private function get_posts_include($post_types, $posts_per_page, $last_days)
    {
    ...
    private function taxonomy_helper()
    {
        ...
        //  здесь исключаем стандартные таксономии из списка всех таксономий таким образом получаем список
        //  только кастомных таксономий
